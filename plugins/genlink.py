...
async def gen_link_s(bot, message):
    ...
    await message.reply(f"Here is your Link:\nhttps://telegram.dog/{temp.U_NAME}?start={outstr}")
    
    
@Client.on_message(filters.command(['batch', 'pbatch']) & filters.create(allowed))
async def gen_link_batch(bot, message):
    if " " not in message.text:
        return await message.reply("ᴜsᴇ ᴄᴏʀʀᴇᴄᴛ ғᴏʀᴍᴀᴛ \nᴇxᴀᴍᴘʟᴇ <code>/batch https://t.me/TeamEvamaria/10 https://t.me/TeamEvamaria/20</code>.")
    ...
    if chat_id in FILE_STORE_CHANNEL:
        ...
        return await sts.edit(f"ʜᴇʀᴇ ʏᴏᴜʀ ʟɪɴᴋ https://telegram.dog/{temp.U_NAME}?start=DSTORE-{b_64}")
    ...
    await sts.edit(f"ʜᴇʀᴇ ɪs ʏᴏᴜʀ ʟɪɴᴋ \nContains `{og_msg}` files.\n https://telegram.dog/{temp.U_NAME}?start=BATCH-{file_id}")
